<svelte:head>
	<title>Críticas - Icônica</title>
</svelte:head>

<ul class="breadcrumb">
	<li class="breadcrumb-item">
		<a href="/" class="btn btn-link">Home</a>
	</li>
	<li class="breadcrumb-item">
		<a href="/critiques" class="btn btn-link">Críticas</a>
	</li>
	<li class="breadcrumb-item">
		<a href="/critiques/critiques_iconica" class="btn btn-link">Críticas publicadas pela Icônica</a>
	</li>
</ul>

<h1>Críticas de Arte Publicadas pela Icônica até abril 2019</h1>
<table class="table">
    <thead>
            <tr>
               <th>Críticas de Arte</th>
               <th>Autores</th>
               <th>Artistas</th>
               <th>Obras</th>
               <th>Exposições</th>
            </tr>
    </thead>
    <tbody>
            <tr>
               <td>DOISNEAU PUBLICITÁRIO</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Robert Doisneau</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>A Renault de Doisneau</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>O OLHAR INCOMUM DE WALKER EVANS</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Walker Evans</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Brooklyn Bridge</li>
                     <li>New York Streets</li>
                     <li>Tahiti</li>
                     <li>Havana</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Walker Evans</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>COLETIVIZANDO O COLETIVO</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Cia de Foto</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>São Paulo de Muitos</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>AVATAR: COISAS QUE A HISTÓRIA DA FOTOGRAFIA ENSINA SOBRE O FUTURO DO CINEMA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>James Cameron</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Avatar</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>GORDON MATTA-CLARK: O REGISTRO COMO OBRA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Gordon Matta-Clark</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Gordon Matta-Clark: Desfazer o espaço</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>O OLHAR E AS PAIXÕES: SOBRE O NOVO FILME DE J. J. CAMPANELLA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Juan Jose Campanella</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>O segredo de seus olhos</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>POR ONDE ANDOU SEBASTIÃO SALGADO?</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Sebastião Salgado</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Gênesis</li>
                     <li>Êxodos</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>POR QUE VER OS CLÁSSICOS</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Maureen Bisilliat</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Série Xingu</li>
                     <li>Série Romeiros</li>
                     <li>Série Pele Preta</li>
                     <li>Série China</li>
                     <li>Série Japão</li>
                     <li>Série As Caranguejeiras</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Maureen Bisilliat – Fotografias</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>O TRISTE FIM DE SOPHIE CALLE</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Sophie Calle</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Cuide de Você</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>IRINA IONESCO: IMAGENS DE UM TEMPO SEM DATA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Irina Ionesco</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Irina Ionesco, Nocturnes Porte Dorée</li>
                     <li>Série Eva: Eloge de ma fille</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Espelho de Sombra e Luz</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>"PIXO" NA BIENAL</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Djan Ivson</li>
                     <li>Rafael Guedes Augustaitiz</li>
                     <li>Adriano Choque</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>29a Bienal de São Paulo</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>DOIS FILMES SOBRE FOTÓGRAFOS</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Philippe Garrel</li>
                     <li>Louis Garrel</li>
                     <li>Laura Smet</li>
                     <li>Philippe Garrel</li>
                     <li>Jan Saudek</li>
                     <li>Sará Saudková</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>A fronteira do alvorecer</li>
                     <li>Jan Saudek: preso por suas paixões, nenhuma chance de resgate</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>UM ANO SEM OTTO STUPAKOFF</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Otto Stupakoff</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>O RETRATO DE ZIDANE</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Douglas Gordon</li>
                     <li>Philippe Parreno</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Zidane, un portrait du 21e siècle</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>VOU AO CINEMA, NÃO ESCAPO DA FOTOGRAFIA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Karin Ainouz</li>
                     <li>Marcelo Gomes</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Viajo porque preciso, volto porque te amo</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>GABINETE DE CURIOSIDADES</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Bob Wolfenson</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Apreensões</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Apreensões</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>LUIZ BRAGA – RUPTURA E CONTEMPLAÇÃO</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Luiz Braga</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Ensaio – Estrada Nova S/N</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>MUITA FOTOGRAFIA E VÍDEO NA BIENAL</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Guy Veloso</li>
                     <li>Jonathas de Andrade</li>
                     <li>Rochelle Costi</li>
                     <li>Rosangela Rennó</li>
                     <li>Miguel Rio Branco</li>
                     <li>Alice Miceli</li>
                     <li>Alfredo Jaar</li>
                     <li>Nan Goldin</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>29a Bienal de São Paulo</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>UM MOMENTO ESPECIAL PARA A FOTOGRAFIA</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Irmãos Vargas</li>
                     <li>Gaspar Gasparian</li>
                     <li>Marcel Gautherot</li>
                     <li>Peter Scheier</li>
                     <li>Thomaz Farkas</li>
                     <li>Bob Wolfenson</li>
                     <li>Miguel Rio Branco</li>
                     <li>Mauro Restiffe</li>
                     <li>Caio Reisewitz</li>
                     <li>Flavia Junqueira</li>
                     <li>Nan Goldin</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Estúdio de Arte Irmãos Vargas – a fotografia em Arequipa, Peru – 1912/1930</li>
                     <li>Gaspar Gasparian – um fotógrafo</li>
                     <li>As construções de Brasília</li>
                     <li>Apreensões</li>
                     <li>Maldicidade</li>
                     <li>Recorrências</li>
                     <li>Esqueceu de Beber Água, Agora Chora de Sede</li>
                     <li>A Casa em Festa</li>
                     <li>18º edição da Coleção Pirelli-Masp</li>
                     <li>Bienal da Internacional</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>CHACAL VS CAPITÃO NASCIMENTO</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Olivier Assayas</li>
                     <li>Brian Eno</li>
                     <li>New Order</li>
                     <li>Wire</li>
                     <li>The Feelies</li>
                     <li>Chico Buarque</li>
                     <li>Pablo Milanez</li>
                     <li>José Padilha</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Carlos</li>
                     <li>Tropa de Elite II</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>A TENSÃO CRÍTICA ENTRE A PALAVRA E A IMAGEM</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Jonathas de Andrade</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Educação para adultos</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>ARQUEOLOGIA DE UM FILME DE GODARD</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Jean-Luc Godard</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Je vous salue, Sarajevo</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>NORMAN ROCKWELL – BEHIND THE CAMERA</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Norman Rockwell</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>First Trip to the Beauty Shop</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Norman Rockwell: Behind the Camera</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>A IMAGEM DO ANO DO WORLD PRESS PHOTO</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Jodi Bieber</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Bibi Aisha, 18</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>World Press Photo of the Year 2011</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>"LA CÁMARA OSCURA", EM BUSCA DE UM OLHAR QUE TRANSCENDE AS APARÊNCIAS</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Maria Victoria Menis</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>La cámara oscura</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>INHOTIM: ESPAÇO E EXPERIÊNCIA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Miguel Rio Branco</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Exposição do acervo do Inhotim em 2011</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>A CONSTRUÇÃO DE UMA GERAÇÃO</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Cláudia Andujar</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Geração 00</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>GERAÇÃO 00</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>João Castilho</li>
                     <li>Claudia Andujar</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Urihi-A</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Geração 00</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>RODRIGO BRAGA NUM SENTIDO EXTRA-MORAL</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Rodrigo Braga</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Mentira repetida</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>TIAGO SANTANA – O BRASIL NA COLEÇÃO PHOTO POCHE</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Tiago Santana</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Sertão</li>
                     <li>Sertão</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>O PROCESSO DE CRIAÇÃO COMO MEMÓRIA</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Cristiano Mascaro</li>
                     <li>Bob Wolfenson</li>
                     <li>Marlene Bergamo</li>
                     <li>Cia de Foto</li>
                     <li>Pio Figueroa</li>
                     <li>Rafael Jacinto</li>
                     <li>João Kehl</li>
                     <li>Carol Lopes</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Bom Retiro e Luz: um roteiro (1976 – 2011)</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>O HABITAT COMO MARCA DISTINTIVA DA ESPÉCIE [OU OS FOTÓGRAFOS NOS EVENTOS DE ARTE]</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>SP-Arte/Foto</li>
                     <li>7º Paraty em Foco - Festival Internacional de Fotografia</li>
                     <li>17º Festival Internacional de Arte Contemporânea Sesc_Videobrasil</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>A IMAGEM DO ANO E AS IMAGENS QUE ATRAVESSAM O TEMPO</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Samuel Aranda</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Fatima al-Qaws cradles her son Zayed (18)</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>World Press Photo of the Year 2012</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>PINA 3D: AS PROFUNDIDADES DO CINEMA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Wim Wenders</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Pina</li>
                     <li>Buena Vista Social Club</li>
                     <li>A invenção de Hugo Cabret</li>
                     <li>Paris Texas</li>
                     <li>Asas do Desejo</li>
                     <li>Alice nas cidades</li>
                     <li>Movimento em falso</li>
                     <li>No decurso do tempo</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>WOLFGANG TILLMANS: ENGENHARIA DO ACASO</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Wolfgang Tillmans</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Wolfgang Tillmans</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>MIS – CENTRO IRRADIADOR DE FOTOGRAFIA</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>André Kertész</li>
                     <li>Claudio Edinger</li>
                     <li>Andy Warhol</li>
                     <li>Ozualdo Candeias</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>De Bom Jesus a Milagres</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>André Kertész – Uma Vida em Dobro</li>
                     <li>Claudio Edinger – De Bom Jesus a Milagres</li>
                     <li>Andy Warhol Superfície Polaroides (1969-1986)</li>
                     <li>Ozualdo Candeias – Rua do Triumpho</li>
                     <li>Acervo Vivo – Câmeras Fotográficas de Dois Séculos</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>AS FORMAS PRIMITIVAS DE APICHATPONG WEERASETHAKUL</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Apichatpong Weerasethakul</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Tio Boonmee, que Pode Recordar suas Vidas Passadas</li>
                     <li>Mekong Hotel</li>
                     <li>Ashes</li>
                     <li>Primitive</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>VIAGENS, APARÊNCIAS E APARIÇÕES</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>João Luiz Musa</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Vila Prudente</li>
                     <li>24X36</li>
                     <li>Tate Modern, Londres, julho de 2011</li>
                     <li>Boulevard des Italiens, Paris, agosto de 2011</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>João Luiz Musa: 24X36 e Vila Prudente</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>ENTRE MORROS – EXCESSO DE REFERÊNCIA E ABSTRAÇÃO</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Claudia Jaguaribe</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Entre Morros</li>
                     <li>Série Entre Morros</li>
                     <li>Menina na Laje</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>A POSE E O ROSTO: AUGUST SANDER E HANS EIJKELBOOM NA BIENAL</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>August Sander</li>
                     <li>Hans Eijkelboom</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Série Homens do século XX</li>
                     <li>Série Foto Notas</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>30ª edição da Bienal de São Paulo</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>ACONTECIMENTO E IMAGINAÇÃO EM SINCRONIAS FOTOGRÁFICAS</td>
               <td>
                  <ul>
                     <li>Cláudia Linhares Sanz</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Michael Ruetz</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Demonstração no Primeiro de Maio, 1967, Berlim Oriental</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>1968: tempos incômodos</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>CÂMARA LÚCIDA, CÂMARA INSANA [FOTÓGRAFOS NÃO SÃO NORMAIS – PARTE II]</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Paul Thomas Anderson</li>
                     <li>Joaquin Phoenix</li>
                     <li>Philip Seymour Hoffman</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>O mestre</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>MAUREEN BISILLIAT, FOTOGRAFIAS ESCOLHIDAS PARA TRANSFORMAÇÃO</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Maureen Bisilliat</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Adesão: Junção/Ligação/União – fotografias escolhidas para transformação</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>CAO GUIMARÃES: PEQUENOS IMPULSOS PARA GRANDES SALTOS</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Cao Guimarães</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Série Gambiarras</li>
                     <li>Mestres da Gambiarra</li>
                     <li>Quarta-feira de cinzas</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Ver é uma fábula</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>DA CAIXA PRETA AO CUBO BRANCO: CRÔNICA SOBRE A RESSIGNIFICAÇÃO DE UM ESPAÇO</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Mauro Restiffe</li>
                     <li>Carlito Carvalhosa</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Série Obra</li>
                     <li>Sala de Espera</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Obra</li>
                     <li>Sala de Espera</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>COISAS COMO ELAS SÃO</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Armando Prado</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Coisas como elas são</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>PODERES PROVISÓRIOS, IMAGENS IMPERTINENTES</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Orlando Brito</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Poder Provisório</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>A PAREDE: TRADUZIR AS INCERTEZAS DE UMA COLEÇÃO</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Norbert H. Kox</li>
                     <li>Thomas Thompson</li>
                     <li>Hans-Peter Feldmann</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>The Gathering</li>
                     <li>Painting of light</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Le Mur</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>SERTÃO SEM SOL, UMA PAISAGEM COM A POUCA LUZ QUE TEM A NOITE</td>
               <td>
                  <ul>
                     <li>Pio Figueiroa</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Alexandre Severo</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Sertanejos</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>RECONTRO, OS TRAÇOS DE UMA VIAGEM À CHINA</td>
               <td>
                  <ul>
                     <li>Pio Figueiroa</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Lívia Aquino</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Recontro</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>O SAL DA TERRA: SINCRONIZAR A IMAGEM E A VOZ DE UM DRAMA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Wim Wenders</li>
                     <li>Juliano Salgado</li>
                     <li>Sebastião Salgado</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>O Sal da Terra</li>
                     <li>Buena Vista Social Club</li>
                     <li>Pina</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>VOTO DE ANA</td>
               <td>
                  <ul>
                     <li>Pio Figueiroa</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Ana Lira</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Voto!</li>
                     <li>Voto</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>SOBREMARINHOS: PALAVRA-MATÉRIA, SENTIDOS FLUIDOS</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Gilvan Barreto</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Sobremarinhos</li>
                     <li>Moscouzinho</li>
                     <li>o Livro do Sol</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>TRAVESSIAS</td>
               <td>
                  <ul>
                     <li>Rubens Fernandes Junior</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>German Lorca</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Geometria das Sombras</li>
                     <li>Aeroporto</li>
                     <li>Mondrian</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Travessias</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>FALAR DE IMAGENS SEM COMPREENDÊ-LAS: APICHATPONG WEERASETHAKUL</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Apichatpong Weerasethakul</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Cemitério do Esplendor</li>
                     <li>Tio Boonmee</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>PRIMEIRAS LIÇÕES DE HIDROLOGIA – E OUTRAS OBSERVAÇÕES</td>
               <td>
                  <ul>
                     <li>Pio Figueiroa</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Adelaide Ivánova</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Primeiras lições de hidrologia – e outras observações</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>SOBRE PEQUENAS PREGNÂNCIAS: CORAÇÃO DE CACHORRO, DE LAURIE ANDERSON</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Laurie Anderson</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Coração de Cachorro</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>A DIVERSIDADE DAS COISAS QUE SÃO AS MESMAS: ANDREY ZIGNNATTO E VINCENT MAUGER</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Andrey Zignnatto</li>
                     <li>Vincent Mauger</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>O CORPO E A CÂMERA: OSCILAÇÕES NOSTÁLGICAS DE WIM WENDERS EM TORNO DA FOTOGRAFIA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Wim Wenders</li>
                     <li>Milla Jovovich</li>
                     <li>Letizia Battaglia</li>
                     <li>Salgado</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Alice nas cidades</li>
                     <li>Palermo Shooting</li>
                     <li>O Sal da Terra</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>A PERDA E O REENCONTRO: AS VÁRIAS DISTÂNCIAS DAS IMAGENS</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Giselle Beiguelman</li>
                     <li>Robert Capa</li>
                     <li>Gerda Taro</li>
                     <li>David Seymour</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
               <td>
                  <ul>
                     <li>Cinema Lascado</li>
                     <li>A Valise Mexicana</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>WESTWORLD: O BORDEL DAS NARRATIVAS</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Jonathan Nolan</li>
                     <li>Lisa Joy</li>
                     <li>Anthony Hopkins</li>
                     <li>Ed Harris</li>
                     <li>Rodrigo Santoro</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Westworld</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>ARTE, INSTITUIÇÃO E SEGURANÇA: SUBIR ESCADAS SEM SEGURAR NO CORRIMÃO</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Ana Maria Tavares</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Exit</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>No lugar mesmo: uma antologia de Ana Maria Tavares</li>
                  </ul>
               </td>
            </tr>
            <tr>
               <td>RELIGAÇÕES: O ESCURO DA FOTOGRAFIA CONTEMPORÂNEA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Fernando de Tacca</li>
                     <li>Chiara Sengberg</li>
                     <li>Guilherme Albuquerque</li>
                     <li>Tony Queiroga</li>
                     <li>Pio Figueiroa</li>
                     <li>Roberta Figue</li>
                     <li>Marilia Zarattini</li>
                     <li>Claudia Elias</li>
                     <li>Manuel Martins</li>
                     <li>Livia Aquino</li>
                     <li>Hans Georg</li>
                     <li>Suzana Mendes</li>
                     <li>Ailton Machado</li>
                     <li>Fernando Schmitt</li>
                     <li>Denise Camargo</li>
                     <li>Cristiane Oshima</li>
                     <li>André Mortatti</li>
                     <li>Élida Nascimento</li>
                     <li>Caroline Moraes</li>
                     <li>Luísa Guanabara</li>
                     <li>Carlos Goldgrub</li>
                     <li>Alexandre Romero</li>
                     <li>Guilherme Maranhão</li>
                     <li>Rose Steinmetz</li>
                     <li>Leo Caobelli</li>
                     <li>Marcelo Costa</li>
                     <li>Guilherme Tosetto</li>
                     <li>Isis Gasparini</li>
                     <li>Marcelo Paciornik</li>
                     <li>Letícia Lampert</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Alusão</li>
                     <li>Série Oásis de Fatalidade</li>
                     <li>Série Diáfano</li>
                     <li>Estudos para tons de cinza</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>ARTISTAS GENIAIS, NERVOSOS, NEURÓTICOS, PERVERSOS…</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Woody Allen</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Annie Hall</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>REAL, SURREAL, SUL DO REAL</td>
               <td>
                  <ul>
                     <li>Didiana Prata</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Rogério Assis</li>
                     <li>Ciro Girard</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Viagem ao Sul do Real</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>WENDEL MEDEIROS | 50 ANOS DO GOLPE MILITAR DE 64</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Wendel Medeiros</li>
                     <li>Coletivo Aparecidos Políticos</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>50 anos do golpe militar de 64</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>FABRÍCIO ARRIAGA | AZIMUTE</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Fabrício Arriaga</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Azimute</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>CYRO ALMEIDA E MESTRE JÚLIO SANTOS | MEMÓRIA INVOLUNTÁRIA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Cyro Almeida</li>
                     <li>Mestre Júlio Santos</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Memória Involuntária</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>KARINA RAMPAZZO | PERSONA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Karina Rampazzo</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Persona</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>MARIANA ANDRADE | IRIS</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Mariana Andrade</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Iris</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>LARA OVÍDIO | TERRITÓRIOS PERECÍVEIS</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Lara Ovídio</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Territórios Perecíveis</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>SAMY SFOGGIA | RAPID EYE MOVEMENTE / BABABADALGHARAGHTAK(…)!</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Samy Sfoggia</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Rapid Eye Movement</li>
                     <li>Bababadalgha-raghtak(…)!</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
            <tr>
               <td>CLAUDIA TAVARES | UM JARDIM NA FLORESTA</td>
               <td>
                  <ul>
                     <li>Ronaldo Entler</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Claudia Tavares</li>
                  </ul>
               </td>
               <td>
                  <ul>
                     <li>Um Jardim na Floresta</li>
                  </ul>
               </td>
               <td>
                  <ul></ul>
               </td>
            </tr>
    </tbody>
</table>